"""``TunedAPIDataSet`` loads the data from HTTP(S) APIs.
...
class TunedAPIDataSet(AbstractDataSet):
    """``TunedAPIDataSet`` loads the data from HTTP(S) APIs.
    It uses the python requests library: https://requests.readthedocs.io/en/master/
    ...
    def __init__(
        ...
        """Creates a new instance of ``TunedAPIDataSet`` to fetch data from an API endpoint.
